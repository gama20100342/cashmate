...
<?php echo $this->Form->create('Groupaccess', array('class' => 'data-form', 'id' => 'new_url_form')); ?>
	...
	<?php 
			...
			?>
				<label class="checkbox-inline text-uppercase text-success fs-10 bold">
						<input type="checkbox" name="data[Groupaccess][action][]" value="<?php echo $key; ?>"					
						<?php
							if(array_key_exists($key, $_views)):
								echo 'checked="checked"';
							endif;
						?>
						/><div class="m-t-3"><?php echo $view; ?></div>						
				</label>
				<div class="clear"></div>
				<?php 
	...
	<?php echo $this->App->formEnd('Save Changes', '#new_url_form'); ?>
	</div>	
	<div class="clear"></div>	
</div>
