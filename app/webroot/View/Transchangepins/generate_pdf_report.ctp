...
$html .= $this->App->tHead($this->Lang->report_header('change_pin'));

if(!empty($trans)){
	$html .='<tbody>';
	foreach($trans as $key => $t):
		$html .='<tr>';
				$html .= '<td>'.$t['Transchangepin']['posimei'].'</td>';
				$html .= '<td>'.$t['Transchangepin']['cardno'].'</td>';
				$html .= '<td>'.$t['Transchangepin']['accountname'].'</td>';
				$html .= '<td>'.date('Y M d h:i A', strtotime($t['Transchangepin']['transdate'])).'</td>';
				$html .= '<td>'.$t['Transchangepin']['processed_by'].'</td>';
