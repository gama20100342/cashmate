	...
	<fieldset>		
		...
		<?php		
			...
			echo $this->App->showForminputs(array(		
				...
				array('input' => 'card_name', 'label' => 'Name to Appear on Card *', 'class' => 'numbers_and_letters card_name','wrapper' => 'col-md-8 nopadding', 'clear' => 1, 'note' => 'Should not exceed to 25 Characters')
				), true);			
	...
	echo $this->Js->Buffer('
		$(document).ready( function(){
			...
			$(".middlename").focusout( function(){
				$(".card_name").val($(".firstname").val() + " " + $(".middlename").val().slice(0, 1) + ". "  + " " + $(".lastname").val());				
			});
		});
	');
?>
